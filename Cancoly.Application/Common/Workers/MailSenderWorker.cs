...
namespace Cancoly.Application.Common.Workers
{
    public class MailSenderWorker : BackgroundService
    {
        ...
        public MailSenderWorker(IConfiguration configuration, IServiceProvider provider, ILogger<MailSenderWorker> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            using (var scope = _provider.CreateScope())
            {
                ...
                while (!stoppingToken.IsCancellationRequested)
                {
                    try
                    {
                        var alerts = await _unitOfWork.NotificationRepository.Query().Where(x=> x.AllowMail == true && x.MailSent == false && x.UserId != null).ToListAsync();
                        ...
                        if (alerts != null)
                        {
                            foreach (var item in alerts)
                            {
                                if (item != null)
                                {
                                    ...
                                    await SendEmailAsync(user.Email, item.Title, item.MailBody, user);
